...
export class XfaFieldProxy {
    ...
    constructor(name: string, fullPath: string, initialValue?: string | null) {
        this.name = name
    ...
    addItem(displayText: string, exportValue?: string): void {
        this._items.push({
            ...
            exportValue:
                exportValue != null ? String(exportValue) : String(displayText),
        })
    }
    ...
    getResult(): XfaScriptResult | null {
        ...
        if (this.rawValue !== this._initialRawValue) {
            result.rawValue = this.rawValue != null ? String(this.rawValue) : ''
            hasChanges = true
        }
