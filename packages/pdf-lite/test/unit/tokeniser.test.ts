...
describe('PDF tokeniser', () => {
    ...
    it('should preserve leading-dot numbers without adding a leading zero on round-trip', () => {
        // PDFs like those generated by Chromium omit the leading zero for decimals
        // e.g. FontMatrix [.00048828125 0 0 -.00048828125 0 0]
        // toFixed() would inflate these to "0.00048828125", shifting byte offsets
        for (const [input, expected] of [
            ['.5', '.5'],
            ['.00048828125', '.00048828125'],
            ['-.5', '-.5'],
            ['-.00048828125', '-.00048828125'],
            ['0.5', '0.5'], // with leading zero: preserved as-is
            ['-0.5', '-0.5'], // with leading zero: preserved as-is
        ] as [string, string][]) {
            const [token] = stringToPdfTokens(input).filter(
                (t) => t instanceof PdfNumberToken,
            ) as PdfNumberToken[]
            expect(
                new TextDecoder().decode(token.toBytes()),
