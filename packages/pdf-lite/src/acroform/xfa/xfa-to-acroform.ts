...
export class XfaToAcroFormConverter {
    ...
    static convert(
        ...
        // 4. Collect page info from document page tree
        ...
        if (acroFormRef instanceof PdfObjectReference) {
            const existing = acroFormRef.resolve()
            if (existing) {
        ...
        // Track field refs per page index for annotation updates
        const fieldRefsByPage = new Map<number, PdfObjectReference[]>()

        let fieldCount = 0

        for (let pageIdx = 0; pageIdx < template.pages.length; pageIdx++) {
            const page = template.pages[pageIdx]
            const pdfPage = pageArray[pageIdx]
            if (!pdfPage) continue

            // Use absolute dimensions since MediaBox may have inverted Y (e.g. [0,792,612,0])
            const pageWidth = Math.abs(pdfPage.width)
            const pageHeight = Math.abs(pdfPage.height)
            ...
            const yOffset = pageHeight - page.height
            ...
            for (const fieldDef of page.fields) {
                ...
                if (!fieldRefsByPage.has(pageIdx)) {
                    fieldRefsByPage.set(pageIdx, [])
                }
                fieldRefsByPage.get(pageIdx)!.push(fieldObj.reference)
        ...
        if (
            ...
            catalog.content.set('AcroForm', acroFormContainer.reference)
        }

        // 11. Render draw elements and update page annotations
        // We must clone each page dict and add it as a new object for incremental updates
        for (let pageIdx = 0; pageIdx < template.pages.length; pageIdx++) {
            const page = template.pages[pageIdx]
            const pdfPage = pageArray[pageIdx]
            if (!pdfPage) continue

            const hasDraws = page.draws.length > 0
            const pageFieldRefs = fieldRefsByPage.get(pageIdx)
            if (!hasDraws && !pageFieldRefs) continue

            const actualWidth = Math.abs(pdfPage.width)
            const actualHeight = Math.abs(pdfPage.height)
            const yOffset = actualHeight - page.height

            // Clone the page dictionary for modification
            const pageDict = pdfPage.content.clone() as PdfDictionary

            // Normalize MediaBox to standard orientation [0, 0, w, h]
            // Some XFA PDFs use inverted Y (e.g. [0, 792, 612, 0])
            const mediaBox = pdfPage.mediaBox
            if (mediaBox[1] > mediaBox[3] || mediaBox[0] > mediaBox[2]) {
                const normalizedBox = new PdfArray([
                    new PdfNumber(Math.min(mediaBox[0], mediaBox[2])),
                    new PdfNumber(Math.min(mediaBox[1], mediaBox[3])),
                    new PdfNumber(Math.max(mediaBox[0], mediaBox[2])),
                    new PdfNumber(Math.max(mediaBox[1], mediaBox[3])),
                ])
                pageDict.set('MediaBox', normalizedBox)
            }

            // Render draw elements as page content streams
            if (hasDraws) {
                ...
                if (drawStream) {
                    const drawStreamObj = new PdfIndirectObject({
                        content: drawStream,
                    })
                    document.add(drawStreamObj)

                    // Add font resources to page
                    ...
                    // Replace page contents to cover "Please wait..." content
            ...
            if (pageFieldRefs) {
                let annots = pageDict.get('Annots')?.as(PdfArray) as
                    | PdfArray<PdfObjectReference>
                    | undefined
                if (!annots) {
                    annots = new PdfArray<PdfObjectReference>()
                } else {
                    annots = annots.clone() as PdfArray<PdfObjectReference>
                }
                for (const ref of pageFieldRefs) {
                    annots.push(ref)
                }
                pageDict.set('Annots', annots)
            }

            // Add updated page as a replacement object in the incremental update
            const updatedPage = new PdfIndirectObject({
                objectNumber: pdfPage.objectNumber,
                generationNumber: pdfPage.generationNumber,
                content: pageDict,
            })
...
function createFieldWidget(
    ...
    if (fieldDef.combo && fieldDef.type === 'Ch') {
        ...
        flags |= 1 << 18 // Edit bit — allow typing in combo box
    }
    if (flags !== 0) {
...
function buildDrawContentStream(
    ...
    for (const draw of draws) {
        ...
        // Text (clipped to draw box)
        if (draw.text && draw.w > 0 && draw.h > 0) {
            // Clip text to the draw box so wrapped lines don't overflow
            ops.push(`${n(dx)} ${n(dy)} ${n(draw.w)} ${n(draw.h)} re W n`)
            ops.push('BT')
            ...
            // Check if text contains explicit newlines (multi-line draw)
            const textLines = draw.text.split('\n').filter((l) => l.trim())
            ...
            // Only render lines that fit within the box height
            const maxLines = Math.max(
                1,
                Math.floor((draw.h - padding) / leading),
            )
