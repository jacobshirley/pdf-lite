 ...
 * @example
 * ```typescript
 * const { U, UE } = generateUandUe(password, fileKey)
 * ```
 */
export function generateUandUe(
    password: ByteArray,
    fileKey: ByteArray,
): { U: ByteArray; UE: ByteArray } {
    ...
    U.set(computeAlgorithm2bHash(password, userValidationSalt))
    U.set(userValidationSalt, 32)
    U.set(userKeySalt, 40)

    const hash = computeAlgorithm2bHash(password, userKeySalt)
    const UE = aes256CbcNoPaddingEncrypt(hash, fileKey, new Uint8Array(16))
 ...
 * @example
 * ```typescript
 * const { O, OE } = generateOandOe(ownerPassword, U, fileKey)
 * ```
 */
export function generateOandOe(
    password: ByteArray,
    U: ByteArray,
    fileKey: ByteArray,
): { O: ByteArray; OE: ByteArray } {
    const random = getRandomBytes(16)
    const ownerValidationSalt = random.subarray(0, 8)
    const ownerKeySalt = random.subarray(8, 16)

    const O = new Uint8Array(48)
    O.set(computeAlgorithm2bHash(password, ownerValidationSalt, U))
    O.set(ownerValidationSalt, 32)
    O.set(ownerKeySalt, 40)

    const hash = computeAlgorithm2bHash(password, ownerKeySalt, U)

    const OE = aes256CbcNoPaddingEncrypt(hash, fileKey, new Uint8Array(16))
